...
namespace T_Sim{
    ...
    bool Parse::parse_xosc(){
        ...
        tinyxml2::XMLElement* Private = root->FirstChildElement("Storyboard")->FirstChildElement("Init")
        ->FirstChildElement("Private");
        if (Private==nullptr){ 
            std::cout << "Error loading Private" << std::endl;
            return false;
        }
        std::cout << "car name: " << Private->entityRef() << std::endl;
        ...
        std::string speed_val = speed->value();
